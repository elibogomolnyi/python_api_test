...
def test_get_one_post_not_exists(authorized_client, test_posts):
    res = authorized_client.get(f"/posts/343434343434")
    assert res.status_code == 200
...
def test_create_post_published_not_true(authorized_client, test_user, test_posts, title, content):
...
def test_unauthorized_user_create_post(client, test_user, test_posts, title, content):
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    res = authorized_client.delete(f"/posts/4554545")
...
def test_updated_post_non_exist(authorized_client, test_user, test_posts):
    data = {
        "title": "updated title",
        "content": "updated content",
        "id": test_posts[3].id
    }
    res = authorized_client.put(f"/posts/4554545", json = data)
    assert res.status_code == 404